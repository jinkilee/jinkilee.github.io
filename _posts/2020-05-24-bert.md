...
2. 메트릭스 reshape & permutation
메트릭스의 shape을 바꾸는 방법들이다. AI 모델링을 하다보면 shape만 맞추려하다가 그 안의 숫자 배열은 신경쓰지 않게 되는 실수를 저지르곤 한다. 그 부분을 조심해야 하기 때문에 reshape과 permutation을 언급하고 넘어가는 것이다.
```python
# reshape
...
위의 코드를 보면 알 수 있듯이, Batch Normalization은 하나의 Batch의 사이즈가 8개 이면 그 8개의 평균을 이용해서 그 Batch 전체를 normalize한다. Normalize하는 방법은 (x-평균)/분산 아주 기본적인 방법이다. 
...
## BERT#4
`BertLayer`는 `BertAttention`, `BertIntermediate`, `BertOutput`으로 구성돼 있다. `BertIntermediate`, `BertOutput`에서는 별다른 중요한 일이 일어나지 않는다.
- `BertIntermediate`: Linear + Activation
- `BertOutput`: Linear + LayerNormalization

## BERT#5 (BertIntermediate + BertOutput)
