...
tensor([  101,   134, 12226,  3781,  3464, 17758,  2684,   103, 14895, 21006,
        ...
        12226,  3781,  3464,  1104,  1103,  2651,  2427,   124,   114,   117,
         ...
         1115,  1169,  1129, 10297, 11812,  1194,  1105, 18723,  1174,  1112,
         1152,  1132, 14776,   119,  1109,  2438,  1106,  1296,  1642,  2450,
         1113,  1103,  4520,  9544,   103,  1113,  1126,  2510,  1591,   112,
          188,   102])
...
tensor([ -100,  -100,  -100,  -100,  -100,  -100,  -100,   134,  -100,  -100,
         ...
         -100,  -100])
```
위의 데이터 예시에서 label에서 -100이 아닌 부분에 대해서만 예측을 하는 것이 MLM이다. -100이 아닌 부분이 전체의 15%정도에 해당된다. 그 선택된 15%에 해당하는 단어를 순서대로 뽑아내면 아래와 같다.
```python
original_input = [134, 1983, 117,  119, 5763, 112]
masked_input   = [103,  103, 103, 3892,  103, 112]
label          = [134, 1983, 117,  119, 5763, 112]
```
`masked_input`과 `label`을 가지고 loss를 계산하게 되는데, `masked_input`의 3892는 무조건 틀리도록 입력 값을 일부러 조작한 케이스라고 볼 수 있다. 조작한 이유는 언어에서는 반드시 그 단어가 들어가야만 하는 법칙이 없기 때문이다. 그리고 `masked_input`의 112의 경우는 일부러 맞추게 하는 케이스로 해석하면 된다.

정리해보자면, BERT의 MLM에서는 하나의 Input에 대해서 15%를 랜덤으로 골라서 그 부분에 대해서만 prediction을 해서 loss를 계산한다. 그런데 랜덤 선택된 15% 중에서
- 80%는 103(`[MASK]`)로 레이블링 후 원래의 Input에 있던 단어를 맞추도록 하고
- 10%는 일부러 틀리도록 Input값을 조작하고
- 나머지 10%는 무조건 맞출 수 있게 Input과 label의 값을 같게 맞들어준다.
