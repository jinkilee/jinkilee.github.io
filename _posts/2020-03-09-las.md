...
>>> inp = torch.FloatTensor(np.random.random((n_batch, maxlen, embedding_size)))
...
```python
>>> pyramid_lstm = nn.LSTM(2*2*listener_hidden_size, listener_hidden_size, bidirectional=True, batch_first=True)
...
```
B: batch size
LH: Listener hidden size
M: Maximum length of sequence
```
| ![Figure 1](http://jinkilee.github.io/img/las/1.png) |
|:--:|
| *Figure 1: Structure of Encoder* |
...
>>> output = torch.FloatTensor(np.zeros((n_batch, 1, n_class)))
...
```
SH: Speaker hidden size
O: Output dimension size
```

| ![Figure 2](http://jinkilee.github.io/img/las/2.png) |
|:--:|
| *Figure 2: Structure of LAS* |
...
```
즉, 오디오 파일 X가 주어졌고 그것을 encoding한 것이 `h`일 때, y의 확률을 구하는 문제인 것이다.  학습을 진행할 때는 ground truth가 있기 때문에 별 문제가 없다. 그런데 문제점이 있다. inference를 할 때, label이 없는데 이럴 경우 prediction이 제대로 안될 수 있다. i번째의 prediction 결과를 i+1에서 사용하기 때문에 한번 틀리면 그 다음부터 주체를 못하는 현상이 발생한다고 이해하면 될 것 같다.

이런 문제를 해결하기 위해서, 학습을 진행할 때 항상 실제 값을 넣지 않고 이전의 문자 분포로부터 sampling을 해서 label을 쓴다. 즉, 약 10% 정도는 정답지(label)을 고의로 바꾼다는 것이다. 왜 굳이 label이 있어도 일부러 틀린 값을 넣을까? Inference 과정에서도 이전 단계의 prediction 값에 크게 영향을 받지 않도록 학습하기 위함이라고 생각하면 될 것 같다.
